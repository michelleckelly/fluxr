...
data {
  ...
  real N2other_daily_mu;
  real N2other_daily_lower;
  real<lower=0> N2other_daily_sigma;
  real Nfix_daily_mu;
  real Nfix_daily_lower;
  real<lower=0> Nfix_daily_sigma;
  real DN_daily_mu;
  real DN_daily_upper;
  ...
  vector[d] light_mult_Nfix[n];
  vector[d] const_mult_N2other[n];
...
parameters {
  vector<lower=N2other_daily_lower>[d] N2other_daily;
  vector<lower=Nfix_daily_lower>[d] Nfix_daily;
...
transformed parameters {
  ...
  vector[d] Nfix_inst[n];
  vector[d] N2other_inst[n];
  vector[d] DN_inst[n];
  vector[d] KN2_inst[n];
  ...
  // * Nfix is linked to light
  // * Nother is constant

  // Calculate individual process rates
  for (i in 1:n) {
    Nfix_inst[i] = Nfix_daily .* light_mult_Nfix[i];
    N2other_inst[i] = N2other_daily .* const_mult_N2other[i];
...
model{
  ...
  N2other_daily ~ normal(N2other_daily_mu, N2other_daily_sigma);
  Nfix_daily ~ normal(Nfix_daily_mu, Nfix_daily_sigma);
...
generated quantities{
  ...
  for(j in 1:d){
    N2other[j] = sum(N2other_inst[1:n, j]) / n24;
    Nfix[j] = sum(Nfix_inst[1:n, j]) / n24;
