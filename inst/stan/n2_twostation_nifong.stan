...
data {
  ...
  vector[d] N2_obs_up[n24];
  vector[d] N2_obs_down[n24];
  vector[d] N2_sat_up[n24];
  vector[d] N2_sat_down[n24];
  vector[d] light_mult_N2consume[n24];
  vector[d] const_mult_DN[n24];
  vector[d] KN2_conv[n24];
  vector[d] depth[n24];
  vector[d] temp[n24];
}
...
transformed parameters {
  ...
  vector[d] N2consume_inst[n24];
  vector[d] DN_inst[n24];
  vector[d] KN2_inst[n24];
  vector[d] N2_mod_down[n24];
...
generated quantities{
  ...
  vector[d] N2consume;
  vector[d] DN;
  vector[n24-daylag] N2_obs_down_vec; // temporary, needed to reorganize matrix structure
  vector[n24-daylag] N2_mod_down_vec; // temporary
  vector[d] N2_R2;
  ...
  for(j in 1:d){
    N2consume[j] = sum(N2consume_inst[daylag:n24, j]) / n24;
    DN[j] = sum(DN_inst[daylag:n24, j]) / n24;
    
    for(i in 1:n24-daylag){
      N2_mod_down_vec[i] = N2_mod_down[i+daylag,j];
      N2_obs_down_vec[i] = N2_obs_down[i+daylag,j];
    }
